...
function fmtCuitLibradorEmail(raw) {
  ...
  const innerHtml = `
    ...
    <div style="background:#f7f7f5;border-radius:10px;padding:18px 20px;margin:0 0 20px 0;">
      <table style="width:100%;font-size:14px;border-collapse:collapse;">
        ...
        <tr><td style="color:#6b7280;padding:6px 0;">CUIT de Librador</td><td style="text-align:right;font-weight:600;">${escapeHtml(fmtCuitLibradorEmail(solicitud.librador))}</td></tr>
...
function buildAprobacion(baseUrl, { email, nombre, solicitud, cuenta, liquidacion, attachments }) {
  ...
  const innerHtml = `
    ...
    <p style="font-size:15px;line-height:1.7;color:#4b5563;margin:0 0 20px 0;">
      Hola ${escapeHtml(nombre || '')}, tu solicitud fue aprobada. Endosá o enviá el echeq al <strong>CUIT</strong> indicado; el <strong>CUIT del librador</strong> es el que cargaste en tu solicitud.
    </p>
    ${liqBlock}
    <div style="background:#f7f7f5;border-radius:10px;padding:18px 20px;margin:0 0 16px 0;">
      <table style="width:100%;font-size:14px;border-collapse:collapse;">
        <tr><td style="color:#6b7280;padding:6px 0;">N° solicitud</td><td style="text-align:right;font-weight:600;">${escapeHtml(solicitud.numero)}</td></tr>
        <tr><td style="color:#6b7280;padding:6px 0;">CUIT de Librador (tu solicitud)</td><td style="text-align:right;font-weight:600;">${escapeHtml(fmtCuitLibradorEmail(solicitud.librador) || '-')}</td></tr>
