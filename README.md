...
4. El envío se hace desde **`/api/email`** (`api/email.js`). El remitente sale de **`RESEND_FROM_EMAIL`** en Vercel (opcional). Si no la definís, se usa **`administracion@pagonacional.com`** (dominio **`pagonacional.com`** verificado en [Resend → Domains](https://resend.com/domains)). Para pruebas sin dominio propio podés usar `RESEND_FROM_EMAIL=onboarding@resend.dev` (con las limitaciones que indique Resend).
